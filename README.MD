...
- make sure email and password are the same (or change the front-end)
  --This test user is already configured in the front end application. we will register and add it to DB
  --next login will automaticaly log us in once we click on "Demo App" btn
...
#### After adding mock data i was getting error like cannot override jobs table

- Solution - Added mongoose.models.Job || mongoose.model('Job', JobSchema) to Jobs model
